...
import { fetchPostsSamePrice } from "../service/Marketing";

const PAGE_SIZE = 4;

const usePostsSamePriceRooms = (basePrice: number) => {
  ...
  const getKey = (pageIndex: number, previousPageData: ApiResponse | null) => {
    ...
    return `/marketing/post/searching?page=${pageIndex + 1}&size=${PAGE_SIZE}&basePrice=${basePrice}`;
  };
  ...
  const fetcher = async (url: string) => {
    ...
    const basePrice = Number(urlParams.get("basePrice"));

    const response: ApiResponse = await fetchPostsSamePrice(
      basePrice,
      page,
      pageSize
    );
...
export default usePostsSamePriceRooms;
