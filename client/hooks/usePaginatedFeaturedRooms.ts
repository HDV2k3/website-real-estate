...
const usePaginatedFeatureRooms = () => {
  ...
  const getKey = (pageIndex: number, previousPageData: ApiResponse | null) => {
    ...
    return `/marketing/post/list-post-featured?page=${pageIndex + 1}&pageSize=${PAGE_SIZE}`;
  };
  ...
  const fetcher = async (url: string) => {
    ...
    const response: ApiResponse = await fetchPostsFeaturedByPage(
      page,
      pageSize
    );
...
export default usePaginatedFeatureRooms;
